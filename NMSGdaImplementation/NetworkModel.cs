...
namespace NMSGdaImplementation
{
	public class NetworkModel
	{
		private readonly IReliableStateManager stateManager;
		private readonly string deltasDictionaryName = "DeltasDictionary";
		private readonly string versionDictionaryName = "VersionDictionary";
		...
		{
			Logger.LogVerbose(String.Format("Getting values for GID = 0x{0:x16}.", globalId));
			...
			try
			{
				...
				Logger.LogVerbose(String.Format("Getting values for GID = 0x{0:x16} succedded.", globalId));
		...
		{
			Logger.LogVerbose($"Getting extent values for entity type = {entityType} .");
			...
			try
			{
				...
				Logger.LogVerbose($"Getting extent values for entity type = {entityType} succedded.");
		...
		{
			Logger.LogVerbose(String.Format("Getting related values for source = 0x{0:x16}.", source));
			...
			try
			{
				...
				Logger.LogVerbose(String.Format("Getting related values for source = 0x{0:x16} succeeded.", source));
		...
		private void InsertEntity(ResourceDescription rd)
		{
			...
			Logger.LogInformation($"Inserting entity with GID ({globalId:x16}).");
			...
			try
			{
				...
				Logger.LogVerbose($"Inserting entity with GID ({globalId:x16}) successfully finished.");
			}
			catch (Exception ex)
			{
		...
		private void UpdateEntity(ResourceDescription rd)
		{
			...
			try
			{
				...
				Logger.LogInformation($"Updating entity with GID ({globalId:x16}).");
				...
				Logger.LogInformation($"Updating entity with GID ({globalId:x16}) successfully finished.");
			}
			catch (Exception ex)
			{
		...
		private void DeleteEntity(ResourceDescription rd)
		{
			...
			try
			{
				...
				Logger.LogInformation($"Deleting entity with GID ({globalId:x16}).");
				...
				Logger.LogVerbose($"Deleting entity with GID ({globalId:x16}) successfully finished.");
			}
			catch (Exception ex)
			{
		...
		{
			List<Delta> result = await ReadAllDeltas();

			if(result.Count == 0)
			{
				Logger.LogInformation("There is no deltas.");
				return;
			}
			...
			foreach (Delta delta in result)
			{
				...
				{
					Logger.LogError($"Error while applying delta (id = {delta.Id}) during service initialization. {ex.Message}", ex);
				}
			}
		}

		private async Task SaveDelta(Delta delta)
		{
			var deltaDictionary = await this.stateManager.GetOrAddAsync<IReliableDictionary<long, Delta>>(this.deltasDictionaryName);
			var versionDictionary = await this.stateManager.TryGetAsync<IReliableDictionary<string, long>>(this.versionDictionaryName);
			
			if (versionDictionary.HasValue)
			{
				using (ITransaction tx = this.stateManager.CreateTransaction())
				{
					var result = await versionDictionary.Value.TryGetValueAsync(tx, "deltaVersion");

					if (result.HasValue)
					{
						long currentVersion = result.Value;
						await versionDictionary.Value.TryUpdateAsync(tx, "deltaVersion", ++currentVersion, currentVersion - 1);
						Logger.LogInformation($"New version of delta is: {currentVersion}.");
					}
					else
					{
						string msg = $"Dictionary is initialized but doesn't have any value. Value will be set to 1.";
						Logger.LogWarning(msg);
						await versionDictionary.Value.AddAsync(tx, "deltaVersion", 1);
					}
			...
			{
				//Novi dictionary
				var reliableDictionary = await this.stateManager.GetOrAddAsync<IReliableDictionary<string, long>>(this.versionDictionaryName);
				using (ITransaction tx = this.stateManager.CreateTransaction())
				{
					await reliableDictionary.AddAsync(tx, "deltaVersion", 1);
					Logger.LogInformation($"New version of delta is: {1}.");
					await tx.CommitAsync();
				}
			}
			...
			using (ITransaction tx = this.stateManager.CreateTransaction())
			{
				long count = await deltaDictionary.GetCountAsync(tx);
				delta.Id = count;
				await deltaDictionary.AddAsync(tx, delta.Id, delta);

				await tx.CommitAsync();
			}
		}

		private async Task<List<Delta>> ReadAllDeltas()
		{
			...
			using (ITransaction tx = this.stateManager.CreateTransaction())
			{
				...
				if(count > 0)
				{
					...
					{
						ConditionalValue<Delta> delta = await deltaDictionary.TryGetValueAsync(tx, i);
						if (delta.HasValue)
						{
							Delta copiedDelta = new Delta(delta.Value);
							result.Add(copiedDelta);
						}
						else
						{
							Logger.LogWarning($"Delta with id {i} is not presented in dictionary.");
						}
					}

					//var iEnumerable = await deltaDictionary.CreateEnumerableAsync(tx);

					//var delta = iEnumerable.GetAsyncEnumerator().Current.Value;
					//Delta copiedDelta = new Delta(delta);
					//result.Add(copiedDelta);

					////Take rest TODO: BETTER
					//while (await iEnumerable.GetAsyncEnumerator().MoveNextAsync(new CancellationToken()))
					//{
					//	delta = iEnumerable.GetAsyncEnumerator().Current.Value;
					//	copiedDelta = new Delta(delta);
					//	result.Add(copiedDelta);
					//}
				}
		...
		public async Task<long> GetVersion()
		{
			...
			if (versionDictionary.HasValue)
			{
				using (ITransaction tx = this.stateManager.CreateTransaction())
				{
					var currentVersion = await versionDictionary.Value.TryGetValueAsync(tx, "deltaVersion");
