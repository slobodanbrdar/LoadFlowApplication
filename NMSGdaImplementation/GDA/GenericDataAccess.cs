...
namespace NMSGdaImplementation.GDA
{
	public class GenericDataAccess : INetworkModelGDAContract
	{
		...
		protected NetworkModel nm = null;
		...
		{
			return await nm.ApplyDelta(delta);
		}
		...
		public async Task<List<ResourceDescription>> IteratorNext(int n, int id)
		{
			try
			{
				List<ResourceDescription> retVal = GetIterator(id).Next(n, nm);
		...
		private int AddIterator(ResourceIterator iterator)
		{
			lock (resourceItMap)
			{
				...
				Logger.LogInformation($"iterator id {iteratorId}");
