using Common.Logger;
using Common.NetworkModelService.ModelException;
using System;
...
namespace Common.NetworkModelService.GenericDataAccess
{
	...
	public class Delta
	{
			...
			foreach (ResourceDescription rd in insertOps)
			{
				...
				if (idOld < 0)
				{
					if (!globaldChanges.ContainsKey(gidOld))
					{
						...
						if (type <= 0)
						{
							throw new ModelException.ModelException(ErrorCode.InvalidDelta, gidOld, 0, string.Format("Invalid DMS type found in insert delta operation ID = 0x{0:x16}.", rd.Id));
						}
		...
		public void SortOperations()
		{
			...
			if (insertOps.Count != insertOpsOrderedNo)
			{
				...
				throw new ModelException.ModelException(ErrorCode.InvalidDelta, exceptionMessage);
			}
